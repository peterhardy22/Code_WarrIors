...
def delete_nth(order,max_e):
    ...
    for number in order:
        # If the number of times that number occurs in the answer_list is less than what Charlie can handle...
        if answer_list.count(number) < max_e:
            # Then add that number to the order.
